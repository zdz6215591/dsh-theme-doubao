...
预览图见 `docs/`：`hero-light.png`（浅色 1440×900）、`hero-dark.png`（深色）、
`hero-pointer.png`（鼠标划过时的散射）、`hero-tall.png`（1440×1300，验证拉高窗口时
人物与点阵不会脱离）、`hero-narrow.png`（1100×720）。
...
- **人物和点阵场都锚在「英雄控制行」上，不锚在区间中点。** 这是「窗口高的时候人物头像
  和点阵会脱离」的解法：两者之间的距离只剩场景自身的几何，跟剩下的高度无关，所以拉高
  窗口只会把整组留白放到上方，不会把两者拆开。
- 舞台 `[data-doubao-stage]` 从零高度的标题行向下长到那排 chip（工作区 / 预设 / 权限）
  的上沿；人物底边就站在那条线上，底部按立绘高度的 8.5% 用 `mask-image` 淡出，遮掉
  立绘自身的横向裁切边。
- 人物**不加任何 CSS 投影**（`filter: none`）：抠好的透明立绘周围再叠一层
  `drop-shadow`，看着就像"抠图没抠干净"留下的一圈灰雾，而这层灰雾是 CSS 画的、
  不是图里的。纵深感交给身后的点阵场。
...
- 空出来的高度通过给滚动容器设 `justify-content: flex-end` 加底部内边距
  ...
  生效，卸载时按原值恢复。
- 点阵场比人物宽（1.7×），底边也压在那排 chip 上，所以鲸鱼的下半身正好落在人物身后，
  露出来的是两侧和头顶上方 —— 像一枚立在人物背后的徽标。
- `[data-doubao-field]` 是一个**只裁剪不绘制**的盒子，高度正好等于可用区间，超大的
  canvas 放在里面，因此点阵场无论画多大都不可能顶出滚动条（纵向、横向都不会）；它的
  底部渐隐也挂在这个盒子上，所以渐隐是按「区间」而不是按画布计量的，离输入框越近越淡。
- 人物与点阵场按可用区间自适应：`ResizeObserver` 每次重算尺寸与位置，窄窗口自动缩小。

### 像素渲染：任何窗口都不糊

立绘是 727×623 的位图，渲染时守三条规则（都在 `hero.js` 的 `layout()` 里）：
...
**有意偏离（配色）**：原站是黑底、粒子用加法混合的白光。浅色界面这么画等于不可见，
而且上游的光照区间下限接近纯黑、中心还有一圈蓝色 bloom，在白色背景上就变成「又蓝又
灰」的花斑。所以 `appearanceFor()` 里把 `shadeMin === shadeMax`（去掉逐粒子的明暗差）
并且 `glow: 0`（去掉中心蓝晕），只留一个统一色调：浅色是浅蓝紫、深色是白。想要回上游
那套立体渐变，把这两个界限改成 0.28 / 2.79（深色）或 0.55 / 1.35（浅色）、`glow: 0.3`
即可，一行的事。
...
| `tools/cutout.mjs` + `tools/pixdump.ps1` | 立绘抠图：三阶多项式拟合蓝底 → 距离抠像 → 乘性投影阴影剔除 → 软边反混合 + 去溢色 → 连通域去投影 |
...
- **素材是位图**：人物立绘来自参考图（1080×1919）抠图后裁到 727×623 内联进 bundle，
  所以 `client/client.js` 约 620 KiB。想更小可以换更小的立绘。
- **首屏布局是改过的**：仅在主题挂载的首屏阶段，滚动容器被临时改成
  `justify-content: flex-end` 并加底部内边距；不装主题时行为完全不变，卸载即恢复。
- **点阵是单色**：为了「更浅的统一颜色」去掉了上游的逐粒子明暗与中心蓝晕；
  想要回立体渐变见上文 `appearanceFor()` 的一行改法。
- **点阵密度**：与上游同为 60×60 采样；因为本场景里鲸鱼画得比原站更大，
  同一采样下点距看起来更疏。想更细可以调 `digitile.js` 的 `SAMPLE` 与 `CELL`
  （两者要同比缩放才能保持鲸鱼世界尺寸不变）。
- **抠图的边界**：投影是用「背景乘以 <1 的系数」这个模型认出来的，所以如果以后换一张
  投影带明显色偏的原图（比如暖色环境光下的阴影），需要调 `tools/cutout.mjs` 的
  `--shadowmin / --shadowmax / --shadowtol`。
